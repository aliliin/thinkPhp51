...
abstract class Rule
{
    ...
    public function parseRule($request, $rule, $route, $url, $option = [], $matches = [])
    {
        ...
        if (is_string($route) && !empty($matches)) {
            ...
            // http://tp51.work/ams/hello 输出
            // string(11) "index/hello" 返回
            //htmlhello,ThinkPHP5 返回
//            dump($route);
        }
    ...
    protected function dispatchModule($request, $route)
    {
        ...
        // 路由到模块/控制器/操作
        $ispatchObj = new ModuleDispatch($request, $this, [$module, $controller, $action], ['convert' => false]);
        /*dump($ispatchObj);
        ...
        if (preg_match_all('/<\w+\??>/', $rule, $matches)) {
            ...
            */
            foreach ($matches[0] as $name) {
                ...
                if (strpos($name, '?')) {
                    $name     = substr($name, 1, -2); /* <name?> => name*/
                    $optional = true;
                } else {
                    $name = substr($name, 1, -1); /* <name> => name */
                }
